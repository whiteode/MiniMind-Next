...
## 函数 `logits_to_log_probs`

> 对应原代码：`def logits_to_log_probs():`（第 25–60 行）
...
前半：chosen 的 ref log_probs
后半：rejected 的 ref log_probs
前半：chosen 的 policy log_probs
后半：rejected 的 policy log_probs
...
> 对应原代码：`def train_epoch():`（第 272–299 行）
...
> 对应原代码：`# ==================== 保存 checkpoint ====================` 段（第 373–390 行）
...
> 对应原代码：`# ========== 1. 初始化分布式训练环境与随机种子 ==========` 段（第 427–432 行）
...
> 对应原代码：`# ========== 2. 创建保存目录、构造模型配置、检测续训 checkpoint ==========` 段（第 433–440 行）
...
> 对应原代码：`# ========== 3. 设置混合精度自动上下文 ==========` 段（第 441–445 行）
...
> 对应原代码：`# ========== 4. 初始化 wandb / swanlab 实验跟踪 ==========` 段（第 446–456 行）
...
> 对应原代码：`# ========== 5. 定义 policy 模型和 reference 模型 ==========` 段（第 457–484 行）
...
> 对应原代码：`# ========== 数据集、采样器、scaler、优化器 ==========` 段（第 485–491 行）
...
> 对应原代码：`# ========== 6. 续训：从 checkpoint 恢复状态 ==========` 段（第 492–500 行）
...
> 对应原代码：`# ========== 7. DDP 封装 policy 模型 ==========` 段（第 501–506 行）
...
> 对应原代码：`# ========== 8. 多 epoch 训练循环 ==========` 段（第 507–522 行）
...
> 对应原代码：`# ========== 9. 清理分布式进程组 ==========` 段（第 523–558 行）
