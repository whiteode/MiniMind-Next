...
> 对应原代码：`def train_epoch():`（第 135–738 行）
...
> 对应原代码：`# ================= 下面都是日志打印和模型保存逻辑 =================` 段（第 739–937 行）
...
如果用了 torch.compile，还要再扒一层,拿到最原始的模型_orig_mod
以半精度 (half) 将权重保存到 CPU 上，节省硬盘空间和显存
保存额外的训练状态（如优化器状态、当前 step），以便断点续训
及时释放内存
显式释放变量，帮助 Python 垃圾回收，防显存泄漏
值得注意的参数：
...
> 对应原代码：`# ========== 1. 初始化环境和随机种子 ==========` 段（第 938–943 行）
...
> 对应原代码：`# ========== 2. 配置目录、模型参数、检查ckp ==========` 段（第 944–970 行）
...
> 对应原代码：`# ========== 4. 配wandb ==========` 段（第 2572–2581 行）
...
> 对应原代码：`# ========== 5. 定义模型、数据、优化器 ==========` 段（第 2582–2734 行）
...
> 对应原代码：`# ========== 6. 从ckp恢复状态 ==========` 段（第 2735–2745 行）
...
> 对应原代码：`# ========== 7. DDP包模型 ==========` 段（第 2746–2760 行）
...
> 对应原代码：`# ========== 9. 清理分布进程 ==========` 段（第 2794–2796 行）
