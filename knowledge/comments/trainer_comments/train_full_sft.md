...
带等级/颜色的日志工具（同时写入文件 + 终端输出）
判断当前进程是否为主进程（rank=0），控制 I/O
模型 checkpoint 保存/恢复（含 optimizer + scaler）
解析分布式环境变量，初始化进程组
设置全局随机种子（确保可复现性）
创建模型实例，可选加载预训练权重
...
> 对应原代码：`def train_epoch():`（第 419–473 行）
...
> 对应原代码：`# ========== 学习率调度 (Cosine Decay + Warmup) ==========` 段（第 474–491 行）
...
> 对应原代码：`# ========== 反向传播（带 GradScaler） ==========` 段（第 509–520 行）
...
> 对应原代码：`# ========== 梯度累积边界：每 accumulation_steps 步更新一次参数 ==========` 段（第 521–554 行）
...
> 对应原代码：`# ========== 保存 checkpoint ==========` 段（第 568–593 行）
...
> 对应原代码：`# ========== SFT 的关键参数说明 ==========` 段（第 594–649 行）
...
> 对应原代码：`# ========== 1. 初始化分布式环境和随机种子 ==========` 段（第 650–676 行）
...
> 对应原代码：`# ========== 2. 配置目录、模型参数、检查 checkpoint ==========` 段（第 677–683 行）
...
> 对应原代码：`# ========== 4. 配 wandb ==========` 段（第 695–703 行）

**注释：**

```text
========== 4. 配 wandb ==========
SwanLab 是 wandb 的开源替代品
```
...
> 对应原代码：`# ========== 5. 定义模型、数据、优化器 ==========` 段（第 704–717 行）
...
> 对应原代码：`# ========== SFTDataset：SFT 特有的数据封装 ==========` 段（第 718–746 行）
...
> 对应原代码：`# ========== 6. 从 checkpoint 恢复状态 ==========` 段（第 747–758 行）
...
> 对应原代码：`# ========== 7. DDP 包装模型 ==========` 段（第 759–766 行）
