...
> 对应原代码：`class MiniMindConfig:`（第 7–13 行）
...
## 函数 `MiniMindConfig.__init__`

> 对应原代码：`def __init__():`（第 14–1110 行）
...
序列起始符 (Begin of Sequence) 的 token ID
序列结束符 (End of Sequence) 的 token ID
...
前馈神经网络(FFN)中使用的激活函数，通常配合 SwiGLU 使用
...
FFN 的中间层维度，若为 None 则会在后续代码中自动计算 (通常是 hidden_size 的 8/3 倍)
...
多头注意力进化史:
  ...
  • 基线             • 但训练不稳定      • 平衡点选择
词表大小
RMSNorm 为了防止除零错误引入的微小常数 epsilon
旋转位置编码 (RoPE) 的基数 theta (常见有 10000, 500000, 1000000 等)
"""
═══════════════════════════════════════════════════════════════════════════════
...
是否启用 FlashAttention-2 加速注意力计算，极大地降低显存占用和提升速度
###################################################
以下是混合专家模型 (MoE) 的专属配置
当 use_moe 为 False 时，以下配置不会生效
###################################################
是否启用 MoE (Mixture of Experts) 架构
...
每个 token 在路由时选择激活的专家数量 (Top-K 路由的 K 值)
...
共享专家的数量 (类似 DeepSeek-MoE 架构，所有 token 都会经过共享专家以保持通用知识)
...
门控网络 (Gate) 计算路由权重的评分函数，默认为 softmax
...
辅助损失 (Auxiliary Loss) 的权重因子，用于防止 MoE 路由坍塌（即所有 token 都只去少数几个专家）
是否在序列(Sequence)级别上计算辅助损失，否则在整个 Batch 级别上计算
...
模型预训练时的原始最大长度
注意力温度缩放因子
使用 YaRN (Yet another RoPE extensioN) 外推方法
type: str
...
MoE 具体配置赋值
###################################################
是否使用混合专家（Mixture of Experts, MoE）结构
每个 token 选择的专家数量（top-k 路由）
路由专家的总数量（参与动态选择的专家数）
共享专家的数量（所有 token 都会经过的固定专家）
用于计算专家选择得分的函数（如 softmax、sigmoid 等）
辅助损失的权重系数，用于平衡负载均衡损失
是否使用基于序列的辅助损失（如考虑 token 序列分布）
是否对 top-k 选择的专家概率进行归一化处理
📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘
                                            MiniMind Model
📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘📘
```
...
> 对应原代码：`class RMSNorm:`（第 1111–1115 行）
...
> 对应原代码：`def __init__():`（第 1116–1121 行）
...
> 对应原代码：`def forward():`（第 1128–1133 行）
...
## 函数 `precompute_freqs_cis`

> 对应原代码：`def precompute_freqs_cis():`（第 1134–1370 行）
...
构造长度为 end 的位置索引 t
例：如果 end=8192，则 t 是一个从 0 到 8191 的整数序列，形状为 (8192,)。每个元素 t[i] 就代表了位置 i 的索引。这个位置索引将被用来计算每个位置对应的旋转角度（频率乘以位置），从而生成 RoPE 的正余弦编码。
将位置 t 乘上预计算的频率，得到 (end, dim//2) 的矩阵
将频率拼接以匹配 dim 维度，生成实部 (cos) 和虚部 (sin)
注意力缩放因子 (attn_factor) —— YaRN 的精髓
==============================================================================
...
## 函数 `apply_rotary_pos_emb`

> 对应原代码：`def apply_rotary_pos_emb():`（第 1371–1374 行）
...
> 对应原代码：`def rotate_half():`（第 1375–1384 行）
...
> 对应原代码：`def repeat_kv():`（第 1385–1398 行）
...
> 对应原代码：`class Attention:`（第 1399–1403 行）
...
## 函数 `Attention.__init__`

> 对应原代码：`def __init__():`（第 1404–1454 行）
...
> 对应原代码：`def forward():`（第 1455–1562 行）
...
因果掩码，确保每个位置只能关注之前的位置，适用于自回归生成任务
...
> 对应原代码：`class FeedForward:`（第 1563–1567 行）
...
## 函数 `FeedForward.__init__`

> 对应原代码：`def __init__():`（第 1568–1640 行）
...
## 函数 `FeedForward.forward`

> 对应原代码：`def forward():`（第 1641–1646 行）
...
> 对应原代码：`class MoEGate:`（第 1647–1651 行）
...
> 对应原代码：`def __init__():`（第 1652–1819 行）
...
激活数
总专家数
打分函数配置，比如softmax/Sigmoid/gumbel-softmax
...
Kaiming 均匀初始化（何恺明初始化）
```

---

## 函数 `MoEGate.reset_parameters`

> 对应原代码：`def reset_parameters():`（第 1820–2089 行）
...
> 对应原代码：`def forward():`（第 2090–2114 行）

**注释：**

```text
hidden_states 形状: [batch_size, seq_len, hidden_size]
将输入展平为 [batch_size * seq_len, hidden_size]，方便后续矩阵乘法计算每个 token 的专家分数
计算每个 token 在各个专家上的 logits
形状: [batch_size * seq_len, n_experts]
形状: [batch_size * seq_len, n_experts]，每行是一个 token 在所有专家上的概率分布
...
> 对应原代码：`# ========== 计算辅助损失 (Auxiliary Loss) ==========` 段（第 2115–2265 行）
...
mean(0) 是对所有 token 求平均，得到每个专家的平均选中频率
...
> 对应原代码：`class MOEFeedForward:`（第 2266–2270 行）
...
## 函数 `MOEFeedForward.__init__`

> 对应原代码：`def __init__():`（第 2271–2287 行）
...
## 函数 `MOEFeedForward.forward`

> 对应原代码：`def forward():`（第 2288–2342 行）

**注释：**

```text
使用门控机制获取 Top-K 专家的索引、权重以及辅助损失
将输入展平为 [batch_size * seq_len, hidden_size]，方便后续根据专家索引进行分发计算
...
## 函数 `MOEFeedForward.moe_infer`

> 对应原代码：`def moe_infer():`（第 2343–2433 行）
...
> 对应原代码：`class MiniMindBlock:`（第 2434–2438 行）
...
## 函数 `MiniMindBlock.__init__`

> 对应原代码：`def __init__():`（第 2439–2454 行）
...
## 函数 `MiniMindBlock.forward`

> 对应原代码：`def forward():`（第 2455–2469 行）
...
> 对应原代码：`class MiniMindModel:`（第 2470–2474 行）
...
## 函数 `MiniMindModel.__init__`

> 对应原代码：`def __init__():`（第 2475–2494 行）
...
## 函数 `MiniMindModel.forward`

> 对应原代码：`def forward():`（第 2495–2539 行）
...
代表当前输入序列的起始位置（偏移量），如果没有缓存则默认为 0
Token 嵌入映射
根据当前的上下文长度切片提取相应的 RoPE 预计算矩阵
逐层前向传播
present 指的就是当前层计算完、包含了最新 Token 之后的最新 KV Cache（键值缓存）。
进行最后的归一化
...
## 类 `MiniMindForCausalLM`

> 对应原代码：`class MiniMindForCausalLM:`（第 2540–2547 行）
...
## 函数 `MiniMindForCausalLM.__init__`

> 对应原代码：`def __init__():`（第 2548–2560 行）
...
## 函数 `MiniMindForCausalLM.forward`

> 对应原代码：`def forward():`（第 2561–2596 行）

**注释：**

```text
用于在训练/推理时节省内存，只保留序列后部的 logits
主干网络特征提取
...
把模型计算出来的所有零散结果，打包成一个结构化的标准对象输出，并附加外置的辅助损失。
```
