...
```bash
python scripts/Trainer/train_pretrain.py --from_resume 1
python scripts/Trainer/train_full_sft.py --from_resume 1
...
```
...
```bash
python scripts/Deploy/eval_llm.py --weight full_sft # 或 pretrain/dpo/ppo/grpo...
```
...
使用方式直接查看`scripts/Deploy/eval_llm.py`代码即可。
...
```bash
torchrun --nproc_per_node N scripts/Trainer/train_xxx.py
```
...
```bash
# 需要登录: wandb login
torchrun --nproc_per_node N scripts/Trainer/train_xxx.py --use_wandb
# and
python scripts/Trainer/train_xxx.py --use_wandb
```
...
可以选择自己构造词表训练一个“词典”，代码可见`./scripts/Trainer/train_tokenizer.py`（仅供学习参考，若非必要无需再自行训练，MiniMind已自带tokenizer）。
或者选择比较出名的开源大模型分词器，
...
```bash
torchrun --nproc_per_node 1 scripts/Trainer/train_pretrain.py # 1即为单卡训练，可根据硬件情况自行调整 (设置>=2)
# or
python scripts/Trainer/train_pretrain.py
```
...
```bash
torchrun --nproc_per_node 1 scripts/Trainer/train_full_sft.py
# or
python scripts/Trainer/train_full_sft.py
```
...
```bash
torchrun --nproc_per_node 1 scripts/Trainer/train_distillation.py
# or
...
```bash
torchrun --nproc_per_node 1 scripts/Trainer/train_lora.py
# or
...
只需要准备如下格式的对话数据集放置到`./dataset/lora_xxx.jsonl`，启动 `python scripts/Trainer/train_lora.py`
...
python scripts/Deploy/eval_llm.py  --weight dpo --lora_weight lora_medical
```
...
```bash
torchrun --nproc_per_node 1 scripts/Trainer/train_reason.py
# or
...
```bash
torchrun --nproc_per_node 1 scripts/Trainer/train_dpo.py
# or
...
```bash
torchrun --nproc_per_node N scripts/Trainer/train_ppo.py
# or
...
```bash
torchrun --nproc_per_node N scripts/Trainer/train_grpo.py
# or
...
```bash
torchrun --nproc_per_node N scripts/Trainer/train_spo.py
# or
...
```bash
python scripts/Deploy/eval_llm.py --weight full_sft --inference_rope_scaling
```
