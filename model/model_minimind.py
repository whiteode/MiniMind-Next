...
class MiniMindConfig(PretrainedConfig):
    ...
    def __init__(
            ...
            # 唯一的例外：当你拿到一个训练好的大模型，想要用自己的私有小数据集对它进行**微调（Fine-tuning，比如 LoRA 微调）**时，由于你的数据量很小，很容易过拟合。这时候，工程师们通常会把 Dropout 重新打开（比如设为 0.1）。
            ...
            # SiLU 也被称为 Swish 函数（由 Google 提出）。它的数学公式如下：$$f(x) = x \cdot \sigma(x) = \frac{x}{1 + e^{-x}}$$其中 $\sigma(x)$ 是 Sigmoid 函数。为什么用它：在早期的深度学习中，大家最常用的是 ReLU 函数（大于0就是原值，小于0就是0）。相比之下，SiLU 有几个显著的优势：平滑性：它的曲线非常平滑，这在数学上意味着它在训练（求导）时更加稳定。非单调性：在 $x$ 略微小于 0 的区域，SiLU 允许出现一点点负值，而不是像 ReLU 那样直接“一刀切”变成 0。这被证明能帮助模型保留更多的微小梯度，从而在深层网络中表现更好。

            # SwiGLU 结构 (Swish-Gated Linear Unit)

            # 代码注释中提到，SiLU（Sigmoid Linear Unit）通常与 SwiGLU 配合使用。SwiGLU 并不是一个单独的激活函数，而是一种改进版的前馈神经网络（FFN）结构，由学者 Noam Shazeer 提出。

            # 传统 FFN vs. SwiGLU：

            # - 传统 FFN 是串行的处理流程：  
            #   $ x \rightarrow \text{线性层} \rightarrow \text{激活函数} \rightarrow \text{线性层} $
            ...
            # 数学表达式如下：  
            # $$
            # \text{SwiGLU}(x, W_1, W_2) = \text{SiLU}(xW_1) \otimes (xW_2)
            # $$  
            ...
            # - 在 FFN 模块中：就像我们第一节讲的，数据为了进行深度加工，会先从 512 被“膨胀”到 intermediate_size（比如 1365），加工完后，必须再次被压缩回 512 维度。
            ...
            # 这行代码同样常见于使用了 **SwiGLU** 架构的现代大语言模型（如 LLaMA 系列）的配置中。

            # 要理解这个“8/3 倍”的设定，我们需要解答你的两个问题：**中间层指的是哪一层**，以及**为什么偏偏是 8/3**。
            ...
            # 但是在前馈神经网络（FFN）内部，为了让模型能够把特征“展开”去学习更复杂的细节，它会先将数据映射到一个更宽的维度，处理完后再压缩回原来的维度。这个**“更宽的维度”**，就是所谓的**中间层维度 (`intermediate_size`)**。

            # * **形状变化过程**：输入向量 ($d$) $\rightarrow$ **扩大到中间层 ($intermediate\_size$)** $\rightarrow$ 压缩回输出向量 ($d$)。
            # * 你可以把它想象成一个**“两头窄、中间宽”**的橄榄球形状。模型在中间最宽的地方（中间层）进行非线性激活操作。



            # 在结合了上一节提到的 **SwiGLU** 架构后，这里的“中间层”具体对应的是那三个权重矩阵：
            # 1. **Gate 矩阵 ($W_1$)**：把输入从 $d$ 维度映射到 $intermediate\_size$ 维度。
            # 2. **Up 矩阵 ($W_2$)**：同样把输入从 $d$ 维度映射到 $intermediate\_size$ 维度。
            # 3. **Down 矩阵 ($W_3$)**：把经过激活和相乘后的 $intermediate\_size$ 维度的特征，重新降维回 $d$。
            ...
            # 在经典的 Transformer（比如 GPT-2、GPT-3、BERT）中，传统的 FFN 只有两个矩阵，并且**默认中间层是 hidden_size 的 4 倍**。
            # * 参数量计算：两个矩阵的大小都是 $d \times 4d$。
            # * 传统 FFN 总参数量 = $(d \times 4d) + (4d \times d) = \mathbf{8d^2}$。

            # #### B. SwiGLU 带来的“超载”问题
            # 正如上一个回答中提到的，SwiGLU 使用了 **3 个矩阵**（Gate, Up, Down），而不是传统的 2 个。
            # 如果我们依然保持传统 Transformer 的 4 倍扩展率：
            # * 3 个矩阵的参数量 = $(d \times 4d) \times 3 = \mathbf{12d^2}$。
            # * 这会导致模型的参数量和计算量凭空暴增 50%！这在训练大模型时是极其昂贵的。

            # #### C. “8/3 魔法”的诞生（数学平衡）
            # 为了让使用 SwiGLU 的模型和传统 Transformer 在**参数量和计算开销上保持一致**（即公平对比），LLaMA 的作者们决定缩小 SwiGLU 的扩展倍率。

            # 我们要让 SwiGLU 的 3 个矩阵的总参数量，等于传统 FFN 的 2 个矩阵的总参数量（$8d^2$）：
            # * 设新的中间层维度为 $I$。
            # * SwiGLU 总参数量 = $3 \times (d \times I) = 3 \cdot d \cdot I$
            # * 让 $3 \cdot d \cdot I = 8d^2$
            # * 推导出：**$I = \frac{8}{3}d$**
            ...
            # 例如，如果 $d = 4096$，那么 $4096 \times (8/3) \approx 10922$。模型往往不会直接用 10922，而是把它调整到最接近的 256 的倍数（比如 11008）。
