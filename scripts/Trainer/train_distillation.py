...
if __name__ == "__main__":
    ...
    add_train_args(parser, save_weight='full_dist', epochs=6, batch_size=32,
                   learning_rate=5e-6, save_interval=100, max_seq_len=340,
    ...
    wandb = init_wandb(
        args,
        f"MiniMind-Distill-S{args.student_hidden_size}T{args.teacher_hidden_size}-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}",
        ckp_data,
    )
    ...
    for epoch, loader, iters, st, _ in for_each_epoch(
        ...
        train_epoch(epoch, loader, iters, teacher_model, lm_config_student, st, wandb, args.alpha, args.temperature)
