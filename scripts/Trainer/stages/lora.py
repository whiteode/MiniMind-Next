"""lora 阶段：LoRA 参数高效微调（冻结基座权重，只训练低秩矩阵，保存轻量 LoRA 权重）。"""
import time
...
STAGE_DEFAULTS = dict(save_dir='models/lora', save_weight='lora_identity', epochs=50, batch_size=32,
...
def train_epoch_lora(ctx, epoch, loader, iters, start_step):
    """LoRA：只对 lora 参数剪裁/优化，保存时只落盘 LoRA 权重（save_lora）。"""
    args, model, optimizer, scaler = ctx.args, ctx.model, ctx.optimizer, ctx.scaler
    autocast_ctx, lm_config, lora_params, wandb = ctx.autocast_ctx, ctx.lm_config, ctx.lora_params, ctx.wandb
...
def run(args):
    ...
    ctx = TrainCtx(args=args, lm_config=lm_config, model=model, tokenizer=tokenizer,
                   optimizer=optimizer, scaler=scaler, lora_params=lora_params,
    ...
    for epoch, loader, iters, st, _ in for_each_epoch(
        ...
        train_epoch_lora(ctx, epoch, loader, iters, st)
